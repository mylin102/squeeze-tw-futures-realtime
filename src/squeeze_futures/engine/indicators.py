...
def calculate_futures_squeeze(df: pd.DataFrame, bb_length=14, bb_std=2.0, kc_length=14, kc_scalar=1.5,
    ...
    if len(df) < min_required:
        # Not enough data, add default columns to prevent errors
        df['sqz_on'] = False
        df['momentum'] = 0.0
        df['vwap'] = df['Close']
    ...
    # Continue with full calculation (we have enough data)
...
def calculate_mtf_alignment(data_dict: dict[str, pd.DataFrame], weights=None) -> dict:
    ...
    for tf, df in data_dict.items():
        ...
        last = df.iloc[-1]
        # Safely get momentum, default to 0 if column doesn't exist or is NaN
