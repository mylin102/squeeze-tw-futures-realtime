...
def run_simulation(ticker="TMF"):
    ...
    console.print(f"Config: {MGMT['lots_per_trade']} lots, Max {MGMT['max_positions']} pos, Day-trade: {MGMT['force_close_at_end']}")
    ...
    try:
        while True:
            ...
            for tf in ["5m", "15m", "1h"]:
                ...
                if not df.empty: 
                    processed_data[tf] = calculate_futures_squeeze(df, bb_length=STRATEGY["length"], kc_length=STRATEGY["length"])
